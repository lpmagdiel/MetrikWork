...
export function hasOvertimeEnabled(team) {
    return getOvertimeLimitHours(team) > 0;
}
...
export function clampToOvertimeLimit(hours, team) {
    ...
    return limit > 0 ? Math.min(numericHours, limit) : 0;
}

export function exceedsOvertimeLimit(hours, team) {
    const limit = getOvertimeLimitHours(team);
    const numericHours = Math.max(0, Number(hours) || 0);
    return limit > 0 ? numericHours > limit : numericHours > 0;
}

export function getOvertimeLimitMessage(team) {
    const limit = getOvertimeLimitHours(team);
    return limit > 0
        ? `El límite de horas extra de este equipo es ${limit}h.`
        : 'Las horas extra están desactivadas para este equipo.';
}
...
export function applyWorkdayOvertimeLimit(workDay, team) {
    ...
    const isOvertimeEntry =
        nextWorkDay.type === 'overtime' ||
        nextWorkDay.timerMode === 'overtime' ||
        overtimeHours > 0;
